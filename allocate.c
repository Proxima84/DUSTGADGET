...
void allocate_commbuffers(void)
{
    ...
    All.BunchSizeForce = (All.BufferSize * 1024 * 1024)
    ...
    All.BunchSizeDensity = (All.BufferSize * 1024 * 1024)
    ...
    All.BunchSizeHydro = (All.BufferSize * 1024 * 1024)
    ...
    All.BunchSizeDrag = (All.BufferSize * 1024 * 1024)
        / (2 * sizeof(struct dragdata_in) + 2 * sizeof(struct dragdata_out));

    DragDataIn = (struct dragdata_in*)CommBuffer;
    DragDataGet = DragDataIn + All.BunchSizeDrag;
    DragDataResult = (struct dragdata_out*)(DragDataGet + All.BunchSizeDrag);
    DragDataPartialResult = DragDataResult + All.BunchSizeDrag;

    All.BunchSizeDomain = (All.BufferSize * 1024 * 1024)
        / (sizeof(struct particle_data) + sizeof(struct sph_particle_data) + sizeof(peanokey));
    ...
    if (ThisTask == 0)
    {
        ...
        printf("Communication buffer has room for %d particles in hydro computation\n",
            All.BunchSizeHydro);
        printf("Communication buffer has room for %d particles in drag decomposition\n",
            All.BunchSizeDrag);
