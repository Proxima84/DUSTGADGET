...
void domain_decompose(void)
{
    ...
    MPI_Allgather(NtypeLocal, 3, MPI_INT, temp, 3, MPI_INT, MPI_COMM_WORLD);
    for (i = 0; i < 3; i++)
    {
        Ntype[i] = 0;
        for (j = 0; j < NTask; j++)
...
int domain_findSplit(int cpustart, int ncpu, int first, int last)
{
    ...
    {
        maxAvgLoad_CurrentSplit = dmax(load_leftOfSplit / ncpu_leftOfSplit,
    ...
    /* we will now have to check whether this solution is possible given the restrictions on the
     * maximum load */
    ...
    if ((ncpu - ncpu_leftOfSplit) >= 2)
        ok_right
            = domain_findSplit(cpustart + ncpu_leftOfSplit, ncpu - ncpu_leftOfSplit, split, last);
    else
        ok_right = 0;
        ...
        for (task = 0, moved = 0; task < NTask - 1; task++)
        {
            ...
            if (list_work[task] < list_work[task + 1])
            {
                ...
                if (newmaxw <= maxw)
                {
                    ...
                    {
                        if (list_loadsph[task] + DomainCountSph[DomainStartList[task + 1]]
                            > maxloadsph)
                            continue;
            ...
            else
            {
                ...
                if (newmaxw <= maxw)
                {
                    ...
                    {
                        if (list_loadsph[task + 1] + DomainCountSph[DomainEndList[task]]
                            > maxloadsph)
                            continue;
...
void domain_exchangeParticles(int partner, int sphflag, int send_count, int recv_count)
{
    ...
    for (rep = 0; rep < 2; rep++)
    {
        if ((rep == 0 && ThisTask < partner) || (rep == 1 && ThisTask > partner))
        {
            if (send_count > 0)
            {
                MPI_Ssend(&DomainPartBuf[0], send_count * sizeof(struct particle_data), MPI_BYTE,
                    partner, TAG_PDATA, MPI_COMM_WORLD);

                MPI_Ssend(&DomainKeyBuf[0], send_count * sizeof(peanokey), MPI_BYTE, partner,
                    TAG_KEY, MPI_COMM_WORLD);

                if (sphflag)
                    MPI_Ssend(&DomainSphBuf[0], send_count * sizeof(struct sph_particle_data),
        ...
        if ((rep == 1 && ThisTask < partner) || (rep == 0 && ThisTask > partner))
        {
            if (recv_count > 0)
            {
                if (sphflag)
                {
                    ...
                    MPI_Recv(&P[N_gas], recv_count * sizeof(struct particle_data), MPI_BYTE,
                        partner, TAG_PDATA, MPI_COMM_WORLD, &status);
                    MPI_Recv(&Key[N_gas], recv_count * sizeof(peanokey), MPI_BYTE, partner, TAG_KEY,
                        MPI_COMM_WORLD, &status);
                    MPI_Recv(&SphP[N_gas], recv_count * sizeof(struct sph_particle_data), MPI_BYTE,
                ...
                {
                    MPI_Recv(&P[NumPart], recv_count * sizeof(struct particle_data), MPI_BYTE,
                        partner, TAG_PDATA, MPI_COMM_WORLD, &status);
...
void domain_sumCost(void)
{
    ...
    MPI_Allreduce(
        local_DomainCountSph, DomainCountSph, NTopleaves, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
void domain_determineTopTree(void)
{
    ...
    MPI_Allgatherv(toplist_local, ntop_local * sizeof(struct topnode_exchange), MPI_BYTE, toplist,
        ntopnodelist, ntopoffset, MPI_BYTE, MPI_COMM_WORLD);
...
void domain_topsplit_local(int node, peanokey startkey)
{
    ...
    if (TopNodes[node].Size >= 8)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            {
                printf("task=%d: We are out of Topnodes. Increasing the constant MAXTOPNODES might "
                       "help.\n",
                    ThisTask);
                fflush(stdout);
                endrun(13213);
