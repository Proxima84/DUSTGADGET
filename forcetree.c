        ...
        while (1)
        {
            ...
            else
            {
                ...
                if (nfreep->len < 1.0e-3 * epsilon)
                {
                     ...
                     * locations. Randomize subnode index to allow tree construction. Note:
                     * Multipole moments
                     * of tree are still correct, but this will only happen well below gravitational
                     * softening
                     * length-scale anyway.
                     */
                    srand((0xffff & P[i].ID) + P[i].GravCost);
                    subnode = (int)(8.0 * rand() / RAND_MAX);
                    P[i].GravCost += 1;
                ...
                if ((numnodes) >= MaxNodes)
                {
                    ...
                    printf("i=%d task=%d ID=%d Type=%d\n hsml = %g pos=(%g|%g|%g)\n", i, ThisTask,
                        (int)P[i].ID, P[i].Type, SphP[i].Hsml, P[i].Pos[0], P[i].Pos[1],
                        P[i].Pos[2]);
                    dump_particles();
                    endrun(1);
                }
...
*/
void force_create_empty_nodes(
    int no, int topnode, int bits, int x, int y, int z, int* nodecount, int* nextfree)
{
            ...
            for (j = 0; j < 2; j++)
                for (k = 0; k < 2; k++)
                {
                    ...
                    Nodes[*nextfree].center[0]
                        = Nodes[no].center[0] + (2 * i - 1) * 0.25 * Nodes[no].len;
                    Nodes[*nextfree].center[1]
                        = Nodes[no].center[1] + (2 * j - 1) * 0.25 * Nodes[no].len;
                    Nodes[*nextfree].center[2]
                        = Nodes[no].center[2] + (2 * k - 1) * 0.25 * Nodes[no].len;
        ...
        if (i < DomainMyStart || i > DomainMyLast)
        {
            ...
            while (1)
            {
                if (th >= All.MaxPart) /* we are dealing with an internal node */
                {
                    ...
                    if (nn
                        >= 0) /* ok, something is in the daughter slot already, need to continue */
                    {
                        th = nn;
                    }
                    else
...
void force_update_node_recursive(int no, int sib, int father)
{
    ...
    if (no >= All.MaxPart && no < All.MaxPart + MaxNodes) /* internal node */
    {
        for (j = 0; j < 8; j++)
            suns[j]
                = Nodes[no].u.suns[j]; /* this "backup" is necessary because the nextnode entry will
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(&DomainMoment[DomainStartList[sendTask]],
                ...
                MPI_BYTE, recvTask, TAG_DMOM, &DomainMoment[DomainStartList[recvTask]],
        ...
        if (i < DomainMyStart || i > DomainMyLast)
        {
            ...
            do
            {
                ...
                for (k = 0; k < 3; k++)
                {
                    if (mm > 0)
                    {
                        Nodes[no].u.d.s[k] = (Nodes[no].u.d.mass * Nodes[no].u.d.s[k]
                                                 + massnew * snew[k] - massold * sold[k])
                            / mm;
                        Extnodes[no].vs[k] = (Nodes[no].u.d.mass * Extnodes[no].vs[k]
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(&DomainTreeNodeLen[DomainStartList[sendTask]],
                (DomainEndList[sendTask] - DomainStartList[sendTask] + 1) * sizeof(FLOAT), MPI_BYTE,
                recvTask, TAG_NODELEN, &DomainTreeNodeLen[DomainStartList[recvTask]],
                (DomainEndList[recvTask] - DomainStartList[recvTask] + 1) * sizeof(FLOAT), MPI_BYTE,
                recvTask, TAG_NODELEN, MPI_COMM_WORLD, &status);
    }
    ...
    for (level = 1; level < (1 << PTask); level++)
    {
        ...
        if (recvTask < NTask)
            MPI_Sendrecv(&DomainHmax[DomainStartList[sendTask]],
                (DomainEndList[sendTask] - DomainStartList[sendTask] + 1) * sizeof(FLOAT), MPI_BYTE,
                recvTask, TAG_HMAX, &DomainHmax[DomainStartList[recvTask]],
                (DomainEndList[recvTask] - DomainStartList[recvTask] + 1) * sizeof(FLOAT), MPI_BYTE,
                recvTask, TAG_HMAX, MPI_COMM_WORLD, &status);
    }
    ...
    while (no >= 0)
    {
        ...
        else
        {
            ...
            if (u < 0.5)
                ...
                fac = mass * h3_inv
                    * (21.333333333333 - 48.0 * u + 38.4 * u * u - 10.666666666667 * u * u * u
