...
void interactionGas()
{
    ...
    free(nbuffer);
    free(noffset);
    double t_st, Ks, eps, tau, soundspeed_a;
    ...
    for (i = 0; i < N_gas; i++)
        if ((P[i].Ti_endstep == All.Ti_Current) && (P[i].Type == 0))
        {
            ...
            else
            {
                ...
                {
                    soundspeed_a = sqrt(GAMMA * SphP[i].Pressure / SphP[i].Density);
                    ...
                    if (SphP[i].DustNumber > 0)
                    {
                        ...
                        t_st = All.DustRho * All.DustSize / SphP[i].GasDensity / soundspeed_a;
                        Ks = SphP[i].DustDensity / t_st;
                        eps = SphP[i].DustDensity / SphP[i].GasDensity;
                        // eps = All.MassTable[1] * SphP[i].DustNumber * 1. / SphP[i].GasNumber/
                        // All.MassTable[0];
                        for (j = 0; j < 3; j++)
                        {
                            ...
                            xn1[j] = (xn[j] + SphP[i].HydroAccelMid[j] * tau)
                                / (1. + (eps + 1.) * tau / t_st);
                            yn1[j] = yn[j] + tau * SphP[i].HydroAccelMid[j];
                            ...
                            SphP[i].DtDragEntropy += Ks / SphP[i].GasDensity * (U_ast[j] - V_n1[j])
                                * (U_ast[j] - V_n1[j]);
#endif
                        }
                    }
...
void interactGas_evaluate(int target, int mode)
{
    ...
    FLOAT* pos;
    double V_a[3], H_a[3], rho_j, rho_a, U_j[3];
    int Ni, Li;
    FLOAT X[3];
    int ix[3];
    if (mode == 0)
    ...
    ix[2] = floor((All.BarrierDistance / 5. + pos[j]) / All.DustGasMechStep * 5.);
    X[2] = -All.BarrierDistance / 5. + (ix[j] + 0.5) * All.DustGasMechStep / 5.;
#endif
    ...
    do
    {
        ...
        // printf("%f %i %i %f\n",pos[0],numngb,0,All.DustGasMechStep);
        for (n = 0; n < numngb; n++)
        {
            j = Ngblist[n];
            rho_j += SphP[j].Density;
            U_j[0] += SphP[j].VelPred[0];
            U_j[1] += SphP[j].VelPred[1];
            U_j[2] += SphP[j].VelPred[2];
        }
        Li = n;
    } while (startnode >= 0);
        ...
        if ((P[i].Ti_endstep == All.Ti_Current) && (P[i].Type == 1))
        {
            ...
            else
            {
                ...
                if ((SphP[i].GasNumber > 0) && (SphP[i].DustNumber > 0))
                {
                    ...
                    SphP[i].GasDensity /= SphP[i].GasNumber;
                    SphP[i].DustDensity /= SphP[i].DustNumber;
                    t_st = All.DustRho * P[i].Radius / SphP[i].GasDensity / SphP[i].SoundSpeedMid;
                    Ks = SphP[i].DustDensity / t_st;
                    // printf("K=%f\n",Ks);
                    eps = SphP[i].DustDensity / SphP[i].GasDensity;
                    // eps = All.MassTable[1] * SphP[i].DustNumber * 1. / SphP[i].GasNumber/
                    ...
                    for (j = 0; j < 3; j++)
                    {
                        ...
                        U_n1[j]
                            = (SphP[i].VelPred[j] + tau * V_ast[j] / t_st + P[i].GravAccel[j] * tau)
                            / (1. + tau / t_st);
...
void interactDust_evaluate(int target, int mode)
{
    ...
    FLOAT* pos;
    // int type;
    double rho_a, V_a[3], H_a[3], U_j[3], soundspeed_a, rho_j;
    int Ni, Li;
    FLOAT X[3];
    int ix[3];
    if (mode == 0)
...
/* initialize variables before SPH loop is started */
    ...
    do
    {
        numngb = ngb_treefind_cell(&X[0], 0.5 * All.DustGasMechStep, &startnode, 0);
        // printf("%f %i %i \n",pos[0],numngb,1);
        // printf("dust_gas h=%f N=%i type=%i\n",All.DustGasMechStep,numngb,type);
        for (n = 0; n < numngb; n++)
        {
            j = Ngblist[n];
            ...
            {
                V_a[k] += SphP[j].VelPred[k];
                H_a[k] += SphP[j].HydroAccel[k]; //+P[j].GravAccel[k];
            }
        }
        Ni += n;
    } while (startnode >= 0);
    ...
    if (Ni > 0)
    {
        ...
        {
            numngb = ngb_treefind_cell(&X[0], 0.5 * All.DustGasMechStep, &startnode, 1);
            // printf("dust_dust h=%f N=%i type=%i\n",All.DustGasMechStep,numngb,type);
            for (n = 0; n < numngb; n++)
            {
                j = Ngblist[n];
                rho_j += SphP[j].Density;
    ...
    do
    {
        ...
        for (n = 0; n < numngb; n++)
        {
            ...
            if (r2 < h_i2)
            {
                ...
                K = P[j].Mass * soundspeed_i / (All.DustRho * All.DustSize) * vdotr
                    / (r2 + 0.001 * h_i2) * wk;
                for (k = 0; k < 3; k++)
                    Ka[k] += K * dpos[k];
                Ke += K * vdotr;
    ...
    do
    {
        ...
        for (n = 0; n < numngb; n++)
        {
            ...
            if (r2 < h_i2)
            {
                ...
                for (k = 0; k < 3; k++)
                    K[k] -= P[j].Mass * soundspeed_j / (All.DustRho * All.DustSize) * vdotr
                        / (r2 + 0.001 * h_i2) * dpos[k] * wk;
            }
        }
    } while (startnode >= 0);
    if (Ni > 0)
