...
void restart(int modus)
{
    ...
    int i, masterTask, groupTask, old_MaxPart, old_MaxNodes;
    ...
    masterTask = (ThisTask / NTask) * NTask;

    for (groupTask = 0; groupTask < NTask; groupTask++)
    {
        if (ThisTask == (masterTask + groupTask)) /* ok, it's this processor's turn */
        {
            ...
            {
                if (system(buf_mv)) /* move old restart files to .bak files */
                {
            ...
            {
                MPI_Bcast(&all_task0, sizeof(struct global_data_all_processes), MPI_BYTE, 0,
                    MPI_COMM_WORLD);
            }
            ...
            {
                printf("it seems you have reduced(!) 'PartAllocFactor' below the value of %g "
                       "needed to load the restart file.\n",
            ...
            if (N_gas > 0)
            {
                ...
                {
                    printf("SPH: it seems you have reduced(!) 'PartAllocFactor' below the value of "
                           "%g needed to load the restart file.\n",
            ...
            {
                printf("Tree storage: it seems you have reduced(!) 'PartAllocFactor' below the "
                       "value needed to load the restart file (task=%d). "
                    ...
                    for (i = 0; i < NumPart; i++)
                        if (Nextnode[i] >= old_MaxPart)
                        {
                            if (Nextnode[i] >= old_MaxPart + old_MaxNodes)
                                Nextnode[i]
                                    += (All.MaxPart - old_MaxPart) + (MaxNodes - old_MaxPart);
                            else
